...
def quantize(block, component, num):
	q = load_quantization_table(component, num)
	return (block / q).round().astype(np.int32)

def load_quantization_table(component, num):
	if component == 'lum':
		if num == 1:
			q = np.array([
				[2, 2, 2, 2, 3, 4, 5, 6],
...
def encode(input_f, output_f, num):
	input_file = f"./bmpshki/{input_f}.bmp"
	...
	for i in range(0, rows, 8):
		for j in range(0, cols, 8):
			...
			for k in range(3):
				...
				quant_matrix = quantize(dct_matrix, 'lum' if k == 0 else 'chrom', num)
	...
	with open("results_jpeg.txt", "a", encoding='UTF-8') as file:
		file.write(f"Таблиця квантування - {num}, зображення - {input_file}\n")
		file.write(f"Розмір вихідного файла: {size_vyhsdnogo} байт\n")
...
class JPEGFileReader:
	...
	def __init__(self, filepath):
		self.__file = open(f'./Results/{filepath}.asf', 'r')
...
def dequantize(block, component, num):
	return block * load_quantization_table(component, num)
...
def decoder(input_file, output_file, original_image, num):
	...
	for block_index in range(blocks_count):
		...
		for c in range(3):
			...
			dct_matrix = dequantize(quant_matrix, 'lum' if c == 0 else 'chrom', num)
			block = idct_2d(dct_matrix)
			npmat[i:i + 8, j:j + 8, c] = block + 128

	image = Image.fromarray(npmat, 'YCbCr')
	image = image.convert('RGB')
	image.save(f'./Results/{output_file}.jpeg')

	size_jpeg = os.path.getsize(f'./Results/{output_file}.jpeg')
	width, height = image.size
	ratio = os.path.getsize(original_image) / size_jpeg
	with open("results_jpeg.txt", "a", encoding='UTF-8') as file:
		file.write(f"Розмір файла JPEG: {size_jpeg} байт\n")
		file.write(f"Розмір зображення JPEG: {width}x{height}\n")
		file.write(f"Коефіцієнт стиснення= {ratio}\n\n")


if __name__ == "__main__":
	files = os.listdir("./bmpshki")
	f_names = [el[:3] for el in files]
	for i in range(1, 3):
		for j in f_names:
			encode(j, f"{j}_{i}", i)
			decoder(f"{j}_{i}", f"{j}_{i}", f"./bmpshki/{j}.bmp", i)