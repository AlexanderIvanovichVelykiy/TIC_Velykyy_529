...
def decode_ac(encoded_data_ac, sequence_length):
    unity, point, alphabet_size, alphabet, probability =[], encoded_data_ac[0], encoded_data_ac[1], encoded_data_ac[2], encoded_data_ac[3]
...
Ентропія: {entropy}\n
___________Арифметичне кодування___________
Дані закодованої AC послідовності: {encoded_data_ac}
Закодована AC послідовність: {encoded_sequence_ac}
Значення bps при кодуванні AC: {bps_ac}
Декодована AC послідовність: {decoded_ac}\n
_____________Кодування Хаффмана_____________
Алфавіт     Код символу""")
...
def save_sequence(sequence, entropy, encoded_data_ac, encoded_sequence_ac, bps_ac, decoded_ac, symbol_code, bps_ch, decoded_ch):
        with open("results_AC_CH.txt", "a", encoding="utf8") as file:
            write_inf(file, sequence, entropy, encoded_data_ac, encoded_sequence_ac, bps_ac, decoded_ac, symbol_code,bps_ch, decoded_ch)
