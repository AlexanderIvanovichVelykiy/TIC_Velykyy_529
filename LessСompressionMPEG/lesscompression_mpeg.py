...
def segmentImage(anchor, blockSize=16):
    h, w = anchor.shape
    hSegments, wSegments = int(h/blockSize), int(w/blockSize)
...
def getAnchorSearchArea(x, y, anchor, blockSize, searchArea):
    ...
    sx, sy = max(0, cx-int(blockSize/2)-searchArea), max(0, cy-int(blockSize/2)-searchArea)
...
def main(anchorFrame, targetFrame, saveOutput, blockSize=64):
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
    ...
    for i in range(0, 3):
        ...
        diffFrame = anchorFrame_c - targetFrame_c
    ...
    plt.title(f'Ступінь стиснення = {round(sum(bitsAnchor) / sum(bitsPredicted), 2)}',
